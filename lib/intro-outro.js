...
export const INTRO_DUR = 1.8
export const OUTRO_DUR = 1.5
// Overhead total que el branding suma al video final (solo si hay logo). WS7 lo
// descuenta del target de la voz para que el VIDEO FINAL (con intro+outro) respete
// el techo estricto target+4 — Javier 2026-06-10.
export const BRANDING_OVERHEAD = INTRO_DUR + OUTRO_DUR
...
export async function addIntroOutro(videoPath, logoPath, workDir, opts = {}) {
  ...
  try {
    ...
    const concatCmd = [
      ...
      '-map "[v]" -map "[a]"',
      // CRF 26 (igual que el resto del pipeline) — CRF 20 inflaba el archivo y
      // superaba el límite de Supabase Storage al re-encodear el video completo.
      '-c:v libx264 -profile:v high -level 4.2 -pix_fmt yuv420p -crf 26 -preset fast',
