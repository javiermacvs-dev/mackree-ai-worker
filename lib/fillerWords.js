...
function isSustainedSound(text) {
  ...
  if (/^[aeiouh]+$/.test(text) && /(.)\1/.test(text)) return true
  if (/^m{2,}$/.test(text)) return true
  return false
}

/**
 * Detecta muletilla alargada que Whisper COLAPSÓ a un solo carácter.
 * Empíricamente, cuando Javier dice "yyyyyyyyy" sostenido por 1.3 segundos,
 * Whisper transcribe solo "y" (1 char) pero la duración (end - start) lo revela.
 *
 * Heurística calibrada con audio real (v9 job c4343fc9):
 *   - "y" duración 0.32s → conjunción legítima (NO cortar)
 *   - "y" duración 0.88s → "yyyy" alargada (SÍ cortar)
 *   - "y" duración 1.30s → "yyyyyyy" muy larga (SÍ cortar)
 *   - "o" duración 0.14s → conjunción
 *   - "o" duración 0.72s → "oooooo" alargada
 *
 * Threshold: 0.5s. Sweet spot entre conjunción normal (~0.2-0.35s) y
 * sostenida real (>0.5s típicamente >0.7s).
 */
function isProlongedSingleChar(word) {
  const t = normalize(word.word)
  if (t.length !== 1) return false
  // Solo vocales españolas + 'y' (que también funciona como vocal sostenida)
  // + 'h' por si Whisper transcribe "hhhhh" como "h"
...
export function detectFillerRanges(words, opts = {}) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    // 4.6. Muletilla alargada COLAPSADA por Whisper a 1 char (y, o, a, e, etc.)
    //      detectada por duración temporal > 0.5s.
    if (isProlongedSingleChar(w)) {
      const dur = (w.end - w.start).toFixed(2)
      ranges.push({ start: w.start, end: w.end, reason: `prolonged:${wText}_${dur}s` })
      counts['prolonged_single'] = (counts['prolonged_single'] ?? 0) + 1
      continue
    }
