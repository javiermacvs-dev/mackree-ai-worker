...
/**
 * Detecta sonidos sostenidos / vocales alargadas que Whisper transcribe como
 * una sola palabra: "eeeeeee" → "Eeeee", "uhhhh" → "Uhhh", "mmmm" → "Mmm".
 * Estos NO matchean con el diccionario simple porque tienen N caracteres.
 *
 * Captura:
 *   - 2+ vocales/h repetidos: ee, aaa, eeh, ahh, ahhh, aaaah, oooh, uhhh
 *   - 2+ m's: mm, mmm, mmmm
 * NO captura:
 *   - "ah", "eh", "oh", "uh", "uhm" → ya están en el diccionario simple
 *   - palabras con consonantes regulares
 */
function isSustainedSound(text) {
  if (text.length < 2) return false
  // Solo vocales/h con al menos un char repetido consecutivo
...
export function detectFillerRanges(words, opts = {}) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    // 4.5. Sonido sostenido (vocal alargada / "eeeeee", "uhhhh", "mmmm").
    //      Whisper los transcribe como una palabra única que no matchea el dict simple.
