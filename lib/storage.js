...
// ── R2 (opcional) ───────────────────────────────────────────────────────────
const R2_BUCKET = process.env.R2_BUCKET || 'chixy-video-jobs'
let _r2 = null
...
async function r2SignGet(key, expiresSec) {
  ...
  return getSignedUrl(r2(), new GetObjectCommand({ Bucket: R2_BUCKET, Key: key }), { expiresIn: Math.min(expiresSec, 604800) })
}
...
export async function deleteAsset(userId, jobId, remoteName) {
  ...
  if (r2Enabled()) {
    try {
      await r2().send(new DeleteObjectCommand({ Bucket: R2_BUCKET, Key: remotePath }))
    } catch (e) {
