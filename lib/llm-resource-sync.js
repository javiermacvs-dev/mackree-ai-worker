...
/** Imagen representativa de un recurso: la imagen tal cual, o un frame del video. */
async function representativeJpeg(item, workDir, i) {
  ...
  if (!isVideo) {
    try { return await readFile(item.filePath) } catch { return null }
  }
  // Video: extraer 1 frame (~a 1s, escalado chico) con ffmpeg (disponible en el worker).
  const out = path.join(workDir, `rsync_${i}.jpg`)
  try {
    await execAsync(
      `ffmpeg -y -ss 1 -i "${fwd(item.filePath)}" -frames:v 1 -vf "scale=512:-1" -q:v 5 "${fwd(out)}"`,
      { timeout: 20000 },
    )
