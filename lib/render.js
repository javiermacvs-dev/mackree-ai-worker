...
async function renderCreate({ workDir, openaiKey, manifest }) {
  ...
  if (words.length > 0) {
    await burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, captionReplacements: manifest?.captionReplacements, whatsappNumber: manifest?.whatsappNumber, contacts: manifest?.contacts })
  } else {
  ...
  if (words.length > 0) {
    try {
      ...
      if (finalWords.length > 0) {
        await burnCaptions({ rawPath: finalRawPath, words: finalWords, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, captionReplacements: manifest?.captionReplacements, whatsappNumber: manifest?.whatsappNumber, contacts: manifest?.contacts })
      } else {
...
async function burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle, captionReplacements, whatsappNumber, contacts }) {
  ...
  // CTA de contacto (regla de marca): badges animados con los datos que el cliente
  // eligió en el checklist (correo/teléfono/web). Preferimos manifest.contacts (nuevo);
  // si no viene, back-compat con whatsappNumber (= solo teléfono). Aparecen cuando el
  // video menciona el canal; fallback: cerca del cierre. Sin datos → no se muestran.
  ...
  if (contactList.length) {
    const mention = fixedWords.find((w) =>
      /whats|wasap|watsap|guats|contact|escr[ií]b|correo|email|web|p[aá]gina/i.test(String(w.word ?? '')))
    let totalDur = 0
    try { totalDur = await getMediaDuration(rawPath) } catch { totalDur = 0 }
    const startSec = mention ? mention.start : Math.max(0, totalDur - 4.5)
    let endSec = startSec + 4
    if (totalDur > 0) endSec = Math.min(endSec, totalDur - 0.05)
    if (endSec > startSec + 0.5) {
      contactCtas = contactList.map((c) => ({ text: formatContactCta(c), startSec, endSec }))
      console.log(`[render] contact CTAs (${contactList.map((c) => c.kind).join(',')}) at ${startSec.toFixed(1)}s (${mention ? 'mention' : 'end-fallback'})`)
    }
  }

  await writeFile(captionsPath, buildASS(fixedWords, subtitleStyle, contactCtas), 'utf8')
