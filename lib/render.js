...
async function renderCreate({ workDir, openaiKey, manifest }) {
  ...
  if (words.length > 0) {
    await burnCaptions({ rawPath: captionBase, words, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, captionReplacements: manifest?.captionReplacements, whatsappNumber: manifest?.whatsappNumber, contacts: manifest?.contacts, visualStyle: manifest?.visualStyle, script: manifest?.script, description: manifest?.description, lang: manifest?.lang, impactTitles: manifest?.impactTitles })
  } else {
  ...
  if (words.length > 0) {
    try {
      ...
      if (finalWords.length > 0) {
        ...
        await burnCaptions({ rawPath: finalRawPath, words: finalWords, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, captionReplacements: manifest?.captionReplacements, whatsappNumber: manifest?.whatsappNumber, contacts: manifest?.contacts, visualStyle: manifest?.visualStyle, script: manifest?.script, description: manifest?.description, lang: manifest?.lang, impactTitles: manifest?.impactTitles })
      } else {
...
async function burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle, captionReplacements, whatsappNumber, contacts, visualStyle, script, description, lang, impactTitles }) {
 ...
 */
export async function runCaptionsFix({ workDir, userId, jobId, newReplacements }) {
  ...
  await burnCaptions({
    ...
    subtitleStyle: manifest?.subtitleStyle, captionReplacements: merged,
  ...
  await writeFile(path.join(workDir, 'job.json'), JSON.stringify({ ...manifest, captionReplacements: merged }, null, 2), 'utf8')
