...
async function burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle, captionReplacements, whatsappNumber, contacts, visualStyle }) {
  ...
  if (cardPath) {
    ...
    cmd2 = [
      ...
      // Capas: video → tarjeta de contacto → SUBTÍTULOS al final (regla inamovible: captions al frente).
      `-filter_complex "[1:v]format=rgba,fade=t=in:st=${cardStart.toFixed(2)}:d=0.4:alpha=1,fade=t=out:st=${fadeOut.toFixed(2)}:d=0.4:alpha=1[cta];` +
        `[0:v][cta]overlay=x=(W-w)/2:y=${cardY}:enable='between(t,${cardStart.toFixed(2)},${cardEnd.toFixed(2)})'[mid];` +
        `[mid]ass='${escaped}'[v]"`,
      '-map "[v]" -map 0:a?',
