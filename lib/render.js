...
function buildASS(words, styleKey) {
...
async function renderCreate({ workDir, openaiKey, manifest }) {
  ...
  for (let i = 0; i < mediaItems.length; i++) {
    ...
    if (item.type === 'image') {
      ...
      fc.push(imageMotionChain(`[${idx}:v]`, `[seg_${label}]`, segDur, variant))
    } else {
      ...
      const tlSpeed = Math.min(60, Math.max(1, origDur / segDur || 1))
      console.log(`[render][create] timelapse ${item.name}: orig=${origDur.toFixed(1)}s slot=${segDur.toFixed(1)}s → ${tlSpeed.toFixed(1)}x`)
      inputs.push(`-i ${q(item.filePath)}`)
      fc.push(
        `[${idx}:v]setpts=(PTS-STARTPTS)/${tlSpeed.toFixed(4)},trim=0:${segDur.toFixed(2)},setpts=PTS-STARTPTS,` +
  ...
  if (words.length > 0) {
    await burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle })
  } else {
  ...
  if (sfxEnabled) {
    try {
      ...
      if (sfxPlacements.length > 0) {
        ...
        fc4Parts.push(
          `${mixLabels.join('')}amix=inputs=${mixLabels.length}:duration=first:dropout_transition=0,` +
  ...
  if (words.length > 0) {
    try {
      ...
      if (finalWords.length > 0) {
        await burnCaptions({ rawPath: finalRawPath, words: finalWords, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle })
      } else {
...
// ──────────────────────────────────────────────────────────────────────────────
async function burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle }) {
  const captionsPath = path.join(workDir, 'captions.ass')
  await writeFile(captionsPath, buildASS(words, subtitleStyle), 'utf8')
