...
async function renderCreate({ workDir, openaiKey, manifest }) {
  ...
  if (words.length > 0) {
    await burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, captionReplacements: manifest?.captionReplacements, whatsappNumber: manifest?.whatsappNumber, contacts: manifest?.contacts, visualStyle: manifest?.visualStyle })
  } else {
...
async function burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle, captionReplacements, whatsappNumber, contacts, visualStyle }) {
  ...
  let cardPath = null, cardStart = 0, cardEnd = 0, cardY = 0, cardDur = 0
  if (contactList.length) {
    const mention = fixedWords.find((w) =>
      /whats|wasap|watsap|guats|contact|escr[ií]b|correo|email|web|p[aá]gina|s[ií]guenos|visita/i.test(String(w.word ?? '')))
    let totalDur = 0
    try { totalDur = await getMediaDuration(rawPath) } catch { totalDur = 0 }
    const startSec = mention ? mention.start : Math.max(0, totalDur - 5)
    let endSec = startSec + 4.5
    if (totalDur > 0) endSec = Math.min(endSec, totalDur - 0.05)
    if (endSec > startSec + 0.6) {
      try {
        const outPng = path.join(workDir, 'contact_card.png')
        const card = await renderContactCard({
        ...
        if (card) {
          ...
          cardStart = startSec
          cardEnd = endSec
          cardDur = totalDur > 0 ? totalDur : endSec + 1
          cardY = Math.round(H * 0.56)   // zona media-baja, sin chocar con los subtítulos
          console.log(`[render] contact card (${contactList.map((c) => c.kind).join(',')}, style=${visualStyle || 'default'}) at ${startSec.toFixed(1)}s (${mention ? 'mention' : 'end-fallback'})`)
        }
      } catch (e) {
        console.warn('[render] contact card render failed (skip):', e?.message ?? e)
      }
    }
  }
  ...
  let cmd2
  if (cardPath) {
    const fadeOut = Math.max(cardStart, cardEnd - 0.4)
    cmd2 = [
      'ffmpeg -y',
      `-i ${q(rawPath)}`,
      `-loop 1 -t ${cardDur.toFixed(2)} -i ${q(cardPath)}`,   // loopear el PNG: sin esto el overlay no aparece (error #11)
      `-filter_complex "[0:v]ass='${escaped}'[base];` +
        `[1:v]format=rgba,fade=t=in:st=${cardStart.toFixed(2)}:d=0.4:alpha=1,fade=t=out:st=${fadeOut.toFixed(2)}:d=0.4:alpha=1[cta];` +
        `[base][cta]overlay=x=(W-w)/2:y=${cardY}:enable='between(t,${cardStart.toFixed(2)},${cardEnd.toFixed(2)})'[v]"`,
      '-map "[v]" -map 0:a?',
