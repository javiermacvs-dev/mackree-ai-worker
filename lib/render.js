...
async function applyContentTrim({ rawPath, words, workDir, anthropicKey, lang, openaiKey }) {
  ...
  // 1c. v26 INAMOVIBLE: WORD-GAP silence cuts (reemplaza silencedetect dB que
  //     fallaba en clips con ambient noise alto). Usa timestamps de Whisper:
  //     si hay gap > 0.4s entre word[i].end y word[i+1].start → cortar.
  //     NO depende del nivel de ruido del clip. Detecta pausas REALES de speech.
  const gaps = detectWordGaps(words, { minGapSec: 0.4, padding: 0.10 })
