...
// WS7 (2026-06-07): ajusta la VOZ para que el video respete la duración elegida.
// Reglas de Javier: PISO DURO = target (el video NUNCA dura menos), techo blando =
// target+4. Como la voz es el master de duración de renderCreate, ajustar la voz ajusta
// el video entero — sin tocar las ~10 referencias a voiceDur del pipeline.
//   - voz > target+4  → acelerar con atempo, tope 1.10x (CALIDAD; preserva el tono).
//   - voz < target    → extender con silencio al final (apad) hasta el target.
//   - voz ∈ [target, target+4] → no se toca (voz natural, calidad máxima).
// Si target inválido (0) → no toca nada (back-compat con jobs sin duration).
// La calibración del guión (SaaS) apunta a target+2 → el caso "corta" es raro.
async function adjustVoiceToTarget(voicePath, voiceDur, targetSec, workDir) {
  if (!targetSec || targetSec <= 0) return { path: voicePath, dur: voiceDur }
  const hi = targetSec + 4
  ...
  try {
    if (voiceDur > hi) {
      ...
      console.log(`[render][create] WS7 voz ${voiceDur.toFixed(1)}s > target+4 (${hi}s) → atempo ${factor.toFixed(3)}x → ${d.toFixed(1)}s`)
      return { path: out, dur: d }
    }
    if (voiceDur < targetSec) {
      await execAsync(
        `ffmpeg -y -i ${q(voicePath)} -af "apad=whole_dur=${targetSec.toFixed(2)}" -c:a libmp3lame -q:a 2 ${q(out)}`,
        { timeout: 60_000 },
      )
      const d = await getMediaDuration(out)
      console.log(`[render][create] WS7 voz ${voiceDur.toFixed(1)}s < target (${targetSec}s) → apad → ${d.toFixed(1)}s`)
      return { path: out, dur: d }
    }
    console.log(`[render][create] WS7 voz ${voiceDur.toFixed(1)}s ∈ [${targetSec},${hi}] → sin ajuste`)
...
async function renderCreate({ workDir, openaiKey, manifest }) {
  ...
  // pasan por renderCreate). renderEdit (footage + voz propia) NO se toca.
  {
    ...
    const adj = await adjustVoiceToTarget(voicePath, voiceDur, targetSec, workDir)
    voicePath = adj.path
    voiceDur = adj.dur
  }
