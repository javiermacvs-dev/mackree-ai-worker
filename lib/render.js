...
async function renderEdit({ workDir, openaiKey, manifest }) {
  ...
  for (let i = 0; i < probed.length; i++) {
    ...
    if (item.type === 'image') {
      ...
      // synth silent audio for this segment
      fc.push(
        `aevalsrc=0:duration=${item.dur.toFixed(2)}:sample_rate=44100:channel_layout=stereo[${aLabel}]`,
      )
      idx++
    } else {
      inputs.push(`-i ${q(item.filePath)}`)
      fc.push(
        `[${idx}:v]setpts=PTS-STARTPTS,scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},` +
          `fps=${FPS},setsar=1,format=yuv420p[${vLabel}]`,
      )
      if (item.hasAudio) {
        fc.push(`[${idx}:a]aresample=44100,asetpts=PTS-STARTPTS[${aLabel}]`)
      } else {
        fc.push(
          `aevalsrc=0:duration=${item.dur.toFixed(2)}:sample_rate=44100:channel_layout=stereo[${aLabel}]`,
        )
  ...
  if (hasMusic) {
    ...
    fc.push(`[acat][music]amix=inputs=2:duration=longest:dropout_transition=0,alimiter=limit=0.95[aout]`)
  } else {
    fc.push(`[acat]alimiter=limit=0.95[aout]`)
  }
