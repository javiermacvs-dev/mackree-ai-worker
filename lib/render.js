...
import { addIntroOutro } from './intro-outro.js'
...
function buildASS(words, styleKey, whatsappCta) {
  ...
  // CTA de WhatsApp (regla de marca): caja verde + número, animada (fade + slide-up),
  // cuando el video menciona WhatsApp. Verde WhatsApp #25D366 = &H0066D325 (BGR).
  if (whatsappCta && whatsappCta.number) {
    styleLines.push(
      `Style: WA,Liberation Sans,68,&H00FFFFFF,&H000000FF,&H0066D325,&H64000000,-1,0,0,0,100,100,0,0,3,26,6,5,40,40,0,1`,
    )
  }
  const header = [
    '[Script Info]',
  ...
  if (whatsappCta && whatsappCta.number) {
    ...
    const y2 = Math.round(H * 0.71)
    lines.push(
      `Dialogue: 1,${sToASS(whatsappCta.startSec)},${sToASS(whatsappCta.endSec)},WA,,0,0,0,,{\\fad(350,350)\\move(${x},${y1},${x},${y2})}WhatsApp  ${whatsappCta.number}`,
    )
  }
...
async function renderCreate({ workDir, openaiKey, manifest }) {
  ...
  for (let i = 0; i < mediaItems.length; i++) {
    ...
    if (item.type === 'image') {
      ...
      // VIDEO CHAIN — timelapse REAL + deshake + unsharp + eq (color)
      // INAMOVIBLE #16 (Javier 2026-05-23): el clip COMPLETO entra en su slot de
      // narración (segDur) acelerándose, en vez de mostrar SOLO los primeros segDur
      // segundos. speed = origDur/segDur (cap 60x, min 1x → un clip más corto que su
      // slot, p.ej. el "reveal" final, se queda a 1x solo). Solo aplica en create mode
      // (clip mudo, la voz en off manda); NUNCA en edit mode (acelerar rompería la voz
      // de la persona + los captions Whisper + los cortes word-gap). Si no se puede
      // medir la duración → 1x (fallback al comportamiento previo, nunca rompe).
      let origDur = 0
  ...
  // v29 INAMOVIBLE (Javier): volumen de música por tipo. commercial 0.12 / personal 0.17.
  // Antes renderCreate tenía 0.06 hardcodeado (valor viejo pre-v29). Alineado con renderEdit.
  const isCommercial = (manifest?.style ?? 'commercial') === 'commercial'
  const musicVol = isCommercial ? 0.12 : 0.17
  ...
  if (words.length > 0) {
    await burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, captionReplacements: manifest?.captionReplacements, whatsappNumber: manifest?.whatsappNumber })
  } else {
    await copyFile(rawPath, outputPath)
  }

  // Pass 3 (v30): overlay imágenes IA en momentos clave — PORTADO desde edit mode.
  // Decisión Javier 2026-05-20: el estilo visual elegido debe verse también en el
  // flujo "Crear video con voz clonada", no solo en "Editar mi video". Mismo gate
  // y mismo overlay que renderEdit (style==='commercial' + words + keys). Fallback graceful.
  ...
  if (aiImagesEnabled) {
    try {
      console.log('[render][create] pass 3: detecting key moments for AI images...')
      ...
      if (moments.length > 0) {
        ...
        const generated = await generateImagesForMomentsParallel(moments, workDir, {
          visualStyle: manifest?.visualStyle,
          aspectRatio: '16:9', // nano-banana dibuja las infografias DERECHAS en horizontal
        })
        const ok = generated.filter((g) => g.success && g.imagePath)
        if (ok.length > 0) {
          console.log(`[render][create] pass 3: overlaying ${ok.length} AI images on output.mp4...`)
          ...
          ok.forEach((g, i) => {
            ...
            // Ilustracion IA en 16:9 montada como CARD centrada con fondo difuminado:
            // nano-banana la dibuja DERECHA en horizontal; en 9:16 acostaba el contenido.
            // Nunca se rota ni se recorta el contenido. Fix 2026-05-24.
            fc3.push(
              `[${i + 1}:v]split=2[bg${i}][fg${i}]`,
              `[bg${i}]scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},boxblur=24:3[bgb${i}]`,
              `[fg${i}]scale=${W}:-1:flags=lanczos[fgs${i}]`,
              `[bgb${i}][fgs${i}]overlay=(W-w)/2:(H-h)/2,format=yuva420p,colorchannelmixer=aa=0.97[img${i}]`,
            )
            const nextLabel = `[v${i}]`
            fc3.push(
          ...
          console.log('[render][create] pass 3 cmd (300):', cmd3.slice(0, 300))
          await execAsync(cmd3, { maxBuffer: 300 * 1024 * 1024, timeout: 300_000 })
          await copyFile(withImagesPath, outputPath)
          console.log('[render][create] pass 3 done — AI images applied')
        }
      } else {
        console.log('[render][create] pass 3 skipped: LLM returned 0 moments')
      }
    } catch (e) {
      console.warn(
        `[render][create] pass 3 (AI images) FAILED, keeping clean output: ${e?.message ?? e}`,
      )
    }
  }
...
async function renderEdit({ workDir, openaiKey, manifest }) {
  ...
  const isCommercial = (manifest?.style ?? 'commercial') === 'commercial'
  // v29 INAMOVIBLE (Javier 2026-05-19 10:30 EDT): "subir un poquito más" tras validar v28.
  // Bump +20%: commercial 0.10 → 0.12, personal 0.14 → 0.17. NO bajar de estos valores.
  // Trayectoria: v27=0.06/0.12 → v28=0.10/0.14 → v29=0.12/0.17. Ver CLAUDE.md sección 5.
  const musicVol = isCommercial ? 0.12 : 0.17
  ...
  if (words.length > 0) {
    try {
      ...
      if (finalWords.length > 0) {
        await burnCaptions({ rawPath: finalRawPath, words: finalWords, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, captionReplacements: manifest?.captionReplacements, whatsappNumber: manifest?.whatsappNumber })
      } else {
...
async function burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle, captionReplacements, whatsappNumber }) {
  ...
  // CTA de WhatsApp (regla de marca): si la empresa tiene número y el video MENCIONA
  // WhatsApp, mostramos el badge animado con el número en ese momento. Si el número
  // está configurado pero el transcriptor no captó "WhatsApp", fallback: mostrarlo al
  // final (donde casi siempre va el CTA). Sin número → no se muestra.
  let whatsappCta = null
  if (whatsappNumber && String(whatsappNumber).trim()) {
    const num = String(whatsappNumber).trim()
    const mention = fixedWords.find((w) => /whats|wasap|watsap|guats/i.test(String(w.word ?? '')))
    let totalDur = 0
    try { totalDur = await getMediaDuration(rawPath) } catch { totalDur = 0 }
    let startSec = mention ? mention.start : Math.max(0, totalDur - 4.5)
    let endSec = startSec + 4
    if (totalDur > 0) endSec = Math.min(endSec, totalDur - 0.05)
    if (endSec > startSec + 0.5) {
      whatsappCta = { number: num, startSec, endSec }
      console.log(`[render] WhatsApp CTA at ${startSec.toFixed(1)}s (${mention ? 'mention' : 'end-fallback'}): ${num}`)
    }
  }

  await writeFile(captionsPath, buildASS(fixedWords, subtitleStyle, whatsappCta), 'utf8')
