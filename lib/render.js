...
import { detectFillerRanges, mergeRanges, buildKeepFilter } from './fillerWords.js'
...
async function renderEdit({ workDir, openaiKey, manifest }) {
  ...
  if (wantCaptions && anyAudio && openaiKey) {
    ...
    try {
      ...
      if (words.length > 0) {
        // Content trim: muletillas + repeticiones + trabazones (determinista)
        // + comienzos titubeantes + tomas falsas (LLM). Skipeable con
        // manifest.contentTrim === false.
...
async function applyContentTrim({ rawPath, words, workDir, anthropicKey, lang, openaiKey }) {
  ...
  // 2. Detección LLM (en paralelo con nada por ahora, pero await directo)
  const llmRanges = await llmDetectFalseStartsAndRetakes(words, anthropicKey)
  const allRanges = mergeRanges([...det.ranges, ...llmRanges])
  ...
  const countsStr = Object.entries(det.counts)
    .map(([k, v]) => `${k}×${v}`)
    .join(' ')
  console.log(
    `[content-trim] cutting ${allRanges.length} ranges = ${removedSec.toFixed(2)}s ` +
      `(deterministic: ${countsStr || 'none'}, llm: ${llmRanges.length})`,
  )
  ...
  // Re-transcribe el clip cortado para que los timestamps de captions
  // coincidan con la nueva timeline. Costo extra: 1 Whisper call (~50 cents).
  const adjustedAudio = path.join(workDir, 'adjusted_audio.m4a')
  try {
    await execAsync(
      `ffmpeg -y -i ${q(trimmedPath)} -vn -c:a aac -b:a 128k ${q(adjustedAudio)}`,
      { timeout: 120_000 },
    )
  } catch (e) {
    console.warn('[content-trim] adjusted audio extract failed:', e?.message ?? e)
    return { path: trimmedPath, words: [], removedSec, ranges: allRanges }
  }

  const adjustedWords = await transcribeWords(adjustedAudio, openaiKey)
  console.log(`[content-trim] re-transcribed: ${adjustedWords.length} words (was ${words.length})`)
