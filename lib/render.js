...
async function renderCreate({ workDir, openaiKey, manifest }) {
  ...
  if (words.length > 0) {
    await burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, captionReplacements: manifest?.captionReplacements })
  } else {
  ...
  if (words.length > 0) {
    try {
      ...
      if (finalWords.length > 0) {
        await burnCaptions({ rawPath: finalRawPath, words: finalWords, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, captionReplacements: manifest?.captionReplacements })
      } else {
...
async function burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle, captionReplacements }) {
