...
async function generateCanvasImages({ workDir, voicePath, voiceDur, manifest, openaiKey }) {
  ...
  // 4. Generar las imágenes en paralelo (full-frame, estilo elegido, orientación del formato).
  console.log(`[render][full-ai] generating ${scenes.length} canvas images (style "${manifest?.visualStyle || 'doodle'}", ${manifest?.format || '9:16'})...`)
  const generated = await generateImagesForMomentsParallel(scenes, workDir, {
    visualStyle: manifest?.visualStyle,
    aspectRatio: formatToAspect(manifest?.format),
  })
  // 5. Renombrar las exitosas a media_01.jpg, media_02.jpg, ... (orden narrativo).
  const ok = generated.filter((g) => g.success && g.imagePath)
  let n = 0
  for (const g of ok) {
    n++
    const dest = path.join(workDir, `media_${String(n).padStart(2, '0')}.jpg`)
    await copyFile(g.imagePath, dest)
  }
  console.log(`[render][full-ai] canvas ready: ${n}/${scenes.length} images → media_*.jpg`)
  return { words }
}
