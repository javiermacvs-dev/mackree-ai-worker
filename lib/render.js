...
import { downloadBrandLogo, uploadAsset, deleteAsset, downloadOneAsset, uploadOutput, readJobManifest } from './storage.js'
import {
  detectFillerRanges,
...
import { detectExcessivePauses, buildBreathDuckFilter } from './voice-pacing.js'
import {
...
import { generateAvatarClip, AVATAR_MODEL_PRO, AVATAR_MODEL_STANDARD } from './kie-avatar.js'
import { detectKeyMoments, detectScriptScenes } from './llm-moments.js'
import { detectImpactTitles, buildTitlesASS } from './impact-titles.js'
...
import { detectSFXMoments, pickVariedSFXFile } from './llm-sfx.js'
import { resolveSubtitleStyle, resolveSubtitlePosition, titleSafeFloor } from './subtitle-styles.js'
...
// Regla inamovible (Javier 2026-07-18): timing de pausas + quitar respiración/
// suspiro residual de la voz clonada. Solo recorta pausas EXCESIVAS (no aplasta
// el ritmo bueno que ya trae la puntuación del guión — ver voice-pacing.js) y
// atenúa (no corta del todo) el remanente de cada pausa que sobrevive, incluidas
// apertura y cierre del clip. Aplica a TODO render con voz TTS (editar con voz,
// generar con IA, avatar) — nunca a renderEdit (voz del footage del cliente).
// Fallback TOTAL: cualquier fallo (Whisper, ffmpeg, sin palabras) → null, el
// caller sigue con la voz intacta. Devuelve las words YA reubicadas para que
// el caption pass las reuse sin re-transcribir.
// Decodifica la voz a PCM s16 mono 16kHz para el análisis de energía del duck
// (fix 2026-08-13). Devuelve { pcm: Int16Array, sr } o null si falla.
async function decodeVoicePcm(voicePath) {
  try {
    const sr = 16000
    const { stdout } = await execAsync(
      `ffmpeg -v error -i ${q(voicePath)} -vn -ac 1 -ar ${sr} -f s16le -`,
      { encoding: 'buffer', maxBuffer: 200 * 1024 * 1024, timeout: 60_000 },
    )
    if (!stdout || stdout.length < 2) return null
    const pcm = new Int16Array(stdout.buffer, stdout.byteOffset, Math.floor(stdout.length / 2))
    return { pcm, sr }
  } catch (e) {
    console.warn('[voice-pacing] decodeVoicePcm falló:', e?.message ?? e)
    return null
  }
}

async function tightenVoicePauses(voicePath, workDir, openaiKey) {
  try {
    const words = await transcribeWords(voicePath, openaiKey)
    if (!words.length) return null
    const totalDur = await getMediaDuration(voicePath)

    const { cutRanges } = detectExcessivePauses(words, totalDur)

    let curPath = voicePath
    let curWords = words
    let curDur = totalDur
    let changed = false

    if (cutRanges.length > 0) {
      const keepExpr = buildKeepFilter(cutRanges, totalDur)
      if (keepExpr) {
        const trimmedPath = path.join(workDir, 'voice_paced.mp3')
        const cmd = [
        ...
        await execAsync(cmd, { maxBuffer: 100 * 1024 * 1024, timeout: 120_000 })
        curPath = trimmedPath
        curWords = adjustWordTimestamps(words, cutRanges)
        curDur = await getMediaDuration(curPath)
        changed = true
        console.log(`[voice-pacing] ${cutRanges.length} pausa(s) excesiva(s) recortada(s): ${totalDur.toFixed(2)}s → ${curDur.toFixed(2)}s`)
      }
    }

    // Fix 2026-08-13 (Javier: "la voz se baja en pedazos"): el duck ya NO confía
    // ciegamente en los timestamps de Whisper — se decodifica el PCM real de la
    // voz y solo se atenúan los tramos genuinamente callados dentro de cada
    // pausa. Sin energía medible NO se atenúa nada (jamás arriesgar voz hablada;
    // el recorte de pausas excesivas de arriba no se ve afectado).
    const energy = await decodeVoicePcm(curPath)
    const duckExpr = energy ? buildBreathDuckFilter(curWords, curDur, energy) : null
    if (!energy) console.warn('[voice-pacing] no se pudo decodificar PCM — se omite la atenuación de respiración por seguridad')
    if (duckExpr) {
      const duckedPath = path.join(workDir, 'voice_paced_duck.mp3')
      const cmd = [
        'ffmpeg -y -hide_banner',
        `-i ${q(curPath)}`,
        `-af "${duckExpr}"`,
      ...
      curDur = await getMediaDuration(curPath)
      changed = true
      console.log('[voice-pacing] respiración/suspiro atenuado en las pausas restantes')
    }

    if (!changed) return null
    return { path: curPath, dur: curDur, words: curWords }
  } catch (e) {
    console.warn('[voice-pacing] falló, se mantiene la voz sin tocar:', e?.message ?? e)
    return null
  }
}
...
export function buildASS(words, styleKey, contactCtas, positionKey, opts = {}) {
  const s = resolveSubtitleStyle(styleKey)
  const CHUNK = s.chunk
  const hasContacts = Array.isArray(contactCtas) && contactCtas.length > 0

  // ── Posición vertical (v79) ────────────────────────────────────────────
  // Sin `positionKey` (o 'bottom') → `pos` es null y todo queda EXACTAMENTE
  // como antes: alignment 2 con el MarginV del preset. Los manifiestos viejos
  // caen acá, así que ningún video ya hecho cambia.
  const pos = resolveSubtitlePosition(positionKey)
  let align = s.align
  let marginV = s.mv
  let inlineTag = '{\\an2}'          // el tag que hasta hoy iba fijo en cada línea

  if (pos) {
    // Piso de seguridad: los títulos de impacto viven arriba (\an8 a H*0.14, y
    // dos líneas llegan a ~0.26·H) y el dato de contacto se dibuja en H*0.16.
    // Ningún subtítulo puede subir dentro de esa franja.
    const floorFrac = titleSafeFloor(W, H, opts.hasTitles !== false)

    if (pos.align === 5) {
      // Centro: libass IGNORA MarginV en los alignments medios (4/5/6), así que
      // la única forma fiable es posicionar el punto a mano con \pos.
      const y = Math.round(H * Math.max(pos.frac, floorFrac + 0.06))
      align = 5
      inlineTag = `{\\an5\\pos(${Math.round(W / 2)},${y})}`
      marginV = 0
    } else if (pos.align === 8) {
      // Arriba: el subtítulo se ancla por su borde superior, así que el margen
      // se mide desde arriba. Se fuerza por debajo de la franja de los títulos.
      const fromTop = Math.max(pos.fromTop ?? 0.34, floorFrac + 0.04)
      align = 8
      marginV = Math.round(H * fromTop)
      inlineTag = '{\\an8}'
    } else {
      // Abajo, más alto de lo normal (tercio inferior). MarginV = distancia
      // desde el borde inferior. Se limita para que el texto no acabe metido en
      // la zona de los títulos ni pegado al borde.
      const maxFrac = 1 - floorFrac - 0.10
      const frac = Math.min(pos.frac, Math.max(maxFrac, 0.05))
      align = 2
      marginV = Math.round(H * frac)
      inlineTag = '{\\an2}'
    }
    if (pos.frac !== null || pos.fromTop) {
      console.log(`[captions] posición '${positionKey}' → align=${align} marginV=${marginV} (H=${H})`)
    }
  }

  const styleLines = [
    `Style: Default,${s.font},${s.size},${s.primary},&H000000FF,${s.outline},${s.back},${s.bold},0,0,0,100,100,0,0,${s.borderStyle},${s.ol},${s.sh},${align},${s.mL},${s.mR},${marginV},1`,
  ]
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    const text = chunk
      ...
      .join(' ')
    lines.push(
      `Dialogue: 0,${sToASS(words[i].start)},${sToASS(words[i].end)},Default,,0,0,0,,${inlineTag}${text}`,
    )
  }
  return lines.join('\n')
}
...
async function generateCanvasImages({ workDir, voicePath, voiceDur, manifest, openaiKey, preWords }) {
  ...
  //    Si ya viene transcrita (voice-pacing la reubicó tras recortar pausas
  //    excesivas), se reusa — evita una 2da llamada a Whisper.
  const words = preWords && preWords.length ? preWords : await transcribeWords(voicePath, openaiKey)
...
// 4ª opción — CLONACIÓN de la persona. Genera un talking-head del cliente (su foto + su voz
// clonada) con KIE Kling Avatar y lo deja como media_01.mp4 = LIENZO único del video. El resto
// del pipeline de renderCreate (captions, títulos de impacto, música, logo, intro/outro,
// contacto) lo envuelve idéntico. Fallback TOTAL: si algo falla → no crea media → renderCreate
// cae a su comportamiento normal (fondo negro si no hay otra media). KIE pide URLs PÚBLICAS:
// la foto ya viene como URL (manifest.avatarImageUrl) y la voz (ya ajustada por WS7) se sube.
async function generateAvatarMedia({ workDir, voicePath, manifest }) {
  const imageUrl = manifest?.avatarImageUrl
  if (!imageUrl) { console.warn('[render][avatar] sin avatarImageUrl — skip'); return false }
  const userId = manifest?.userId, jobId = manifest?.jobId
  if (!userId || !jobId) { console.warn('[render][avatar] falta userId/jobId — skip'); return false }
  // Retención = inmediato: la foto cruda del cliente se borra apenas se usó para
  // generar el avatar (éxito o fallo), NO se conserva en Storage. El nombre real
  // (avatar_photo.jpg / .png) se extrae de la propia URL firmada que mandó el SaaS.
  const photoName = (() => {
    try { return decodeURIComponent(new URL(imageUrl).pathname.split('/').pop() || '') || 'avatar_photo.jpg' }
    catch { return 'avatar_photo.jpg' }
  })()
  try {
    const audioUrl = await uploadAsset(userId, jobId, voicePath, 'avatar_voice.mp3', 'audio/mpeg')
    const model = manifest?.avatarQuality === 'pro' ? AVATAR_MODEL_PRO : AVATAR_MODEL_STANDARD
    const result = await generateAvatarClip(imageUrl, audioUrl, { model })
    if (!result?.url) { console.warn('[render][avatar] generación falló — sin avatar'); return false }
    const dest = path.join(workDir, 'media_01.mp4')
    const res = await fetch(result.url)
    if (!res.ok) throw new Error(`download HTTP ${res.status}`)
    await writeFile(dest, Buffer.from(await res.arrayBuffer()))
    console.log('[render][avatar] talking-head listo → media_01.mp4')
    return true
  } catch (e) {
    console.warn('[render][avatar] error:', e?.message ?? e)
    return false
  } finally {
    await deleteAsset(userId, jobId, photoName)
  }
}
...
async function renderCreate({ workDir, openaiKey, manifest }) {
  ...
  // Regla inamovible (Javier 2026-07-18): timing de pausas + quitar respiración/
  // suspiro residual de la voz. Corre ANTES de avatar/generateMedia para que
  // ambos hereden la voz ya pulida (avatar: mismo audio para el lip-sync;
  // generateMedia: reusa estas words para segmentar escenas — ver preWords).
  let preWords = null
  {
    const paced = await tightenVoicePauses(voicePath, workDir, openaiKey)
    if (paced) {
      voicePath = paced.path
      voiceDur = paced.dur
      preWords = paced.words
    }
  }

  // 4ª opción — clonación de la persona (avatar talking-head). Genera el lienzo del cliente
  // hablando ANTES de findMediaItems (queda como media_01.mp4). Usa la voz YA ajustada (WS7
  // + pacing) para que el lip-sync coincida con la duración final. Gateado por manifest.generateAvatar.
  const avatarMode = manifest?.generateAvatar === true
  if (avatarMode) {
    await generateAvatarMedia({ workDir, voicePath, manifest })
  }
  ...
  // → cero impacto sobre ellos). Reusa el transcript (preWords) para los captions del pass 2.
  if (manifest?.generateMedia === true) {
    const r = await generateCanvasImages({ workDir, voicePath, voiceDur, manifest, openaiKey, preWords })
    preWords = r.words
  }
  ...
  // Total efectivo a repartir entre los segmentos (el fold de xfade lo devuelve a voiceDur).
  const totalEff = wantXfade ? voiceDur + (n - 1) * XFADE_DUR : voiceDur
  ...
  // ── INAMOVIBLE #42 (Javier 2026-08-15): REPARTO PONDERADO del tiempo ────────
  // Antes el tiempo se repartía por partes IGUALES entre todos los recursos
  // (segDur = voiceDur/n). Con 6 fotos + 1 clip de 20s, el clip caía en un slot
  // de 4.3s → timelapse 4.7x → "el video del final no se ve nada" (reporte de
  // Javier sobre el job msulw266ya75vdyjzag, la Ford Bronco). Un clip REAL del
  // cliente tiene más contenido que una foto y merece más pantalla.
  //   · foto → peso 1 (con Ken Burns ~3s alcanza para apreciarla)
  //   · clip del cliente → peso proporcional a su duración real, TOPE 3 fotos
  //   · guard: ninguna foto baja de MIN_IMG_SEC; si el reparto la hundiría, los
  //     pesos de los clips se encogen hacia 1 (degrada al comportamiento previo)
  // Los clips GENERADOS (Opción 3 full-AI y avatar) mantienen peso 1: sus
  // duraciones ya vienen diseñadas por escena y acelerarlos/estirarlos rompería
  // el lip-sync (#32) o el orden narrativo (#24).
  // El cliente puede pedir por chat MÁS o MENOS pantalla para sus clips
  // ("acorta las imágenes para dar más tiempo al video") → manifest.clipEmphasis.
  // Sin el campo → 'balanced' = el default calibrado.
  const CLIP_EMPHASIS_W = { less: 1.0, balanced: 3.0, more: 4.5, much_more: 6.0 }
  const PHOTO_REF_SEC = 3.2   // lo que "vale" una foto
  const MAX_VIDEO_W = CLIP_EMPHASIS_W[manifest?.clipEmphasis] ?? CLIP_EMPHASIS_W.balanced
  const MIN_IMG_SEC = 2.2     // piso duro de una foto
  const origDurs = new Array(orderedMedia.length).fill(0)
  await Promise.all(orderedMedia.map(async (it, i) => {
    if (it.type === 'image') return
    try { origDurs[i] = await getMediaDuration(it.filePath) } catch { origDurs[i] = 0 }
  }))
  const weights = orderedMedia.map((it, i) => {
    if (it.type === 'image') return 1
    const generated = (manifest?.generateMedia === true || avatarMode) && /^media_\d+\.mp4$/.test(it.name ?? '')
    if (generated || !(origDurs[i] > 0)) return 1
    return Math.min(MAX_VIDEO_W, Math.max(1, origDurs[i] / PHOTO_REF_SEC))
  })
  {
    const extra = weights.reduce((s, w) => s + (w - 1), 0)
    const allowed = voiceDur / MIN_IMG_SEC - n
    if (extra > 0 && allowed < extra) {
      const factor = Math.max(0, allowed) / extra
      for (let i = 0; i < weights.length; i++) weights[i] = 1 + (weights[i] - 1) * factor
      console.log(`[render][create] pesos recortados (factor ${factor.toFixed(2)}) para que ninguna imagen baje de ${MIN_IMG_SEC}s`)
    }
  }
  const sumW = weights.reduce((s, w) => s + w, 0) || n
  const segDurEffs = weights.map((w) => (totalEff * w) / sumW)
  if (weights.some((w) => Math.abs(w - 1) > 0.01)) {
    console.log(`[render][create] reparto ponderado: ${segDurEffs.map((d, i) => `${orderedMedia[i]?.name ?? i}=${d.toFixed(1)}s`).join(' ')}`)
  }

  for (let i = 0; i < orderedMedia.length; i++) {
    const item = orderedMedia[i]
    const segDurEff = segDurEffs[i] ?? (totalEff / n)
    const label = `s${i}`
    ...
    if (item.type === 'image') {
      ...
      // El clip de avatar (4ª opción) también es "generado": velocidad NATIVA, sin timelapse
      // ni deshake agresivo (acelerarlo rompería el lip-sync con la voz).
      const isGeneratedClip = (manifest?.generateMedia === true || avatarMode) && /^media_\d+\.mp4$/.test(item.name ?? '')

      // Ya medida arriba para calcular los pesos (#42) — no volver a sondear.
      const origDur = origDurs[i] ?? 0
  ...
  // Fold encadenado de xfade: offset(k) = (suma de los k segmentos previos) - k*D.
  // Con segmentos IGUALES se reduce a k*(segDurEff-D) (fórmula original v37); con el
  // reparto ponderado (#42) cada segmento aporta lo suyo y el total sigue volviendo a
  // voiceDur exacto porque sum(segDurEffs) = voiceDur + (N-1)*D por construcción.
  const xfadeCombine = () => {
    const lines = []
    let acc = `[seg_${segLabels[0]}]`
    let accDur = segDurEffs[0] ?? segDurEff
    for (let k = 1; k < segLabels.length; k++) {
      const off = (accDur - XFADE_DUR).toFixed(3)
      accDur += (segDurEffs[k] ?? segDurEff) - XFADE_DUR
  ...
  const minSegEff = segDurEffs.length ? Math.min(...segDurEffs) : segDurEff
  const canXfade = wantXfade && segLabels.length >= 2 && minSegEff > XFADE_DUR + 0.1
  if (canXfade) {
    try {
  ...
  const aiImagesEnabled =
    ...
    avatarMode !== true &&              // 4ª opción: las ilustraciones full-screen taparían la cara
    ...
    try {
      ...
      if (moments.length > 0) {
        ...
        const generated = await generateImagesForMomentsParallel(moments, workDir, {
          visualStyle: manifest?.visualStyle,
          aspectRatio: formatToAspect(manifest?.format), // ratio DEL VIDEO (9:16/1:1/16:9) — animaciones simples llenan el frame en cualquier formato
        })
  ...
  // Persistir la base LIMPIA (sin captions/títulos/contacto) + el transcript ANTES
  // de quemar captions — permite corregir SOLO subtítulos después sin rehacer el
  // video completo (ver runCaptionsFix). No bloqueante: si falla, el render sigue
  // igual, solo no queda disponible la corrección liviana para este job.
  await persistPrecaptionsBase({ userId: manifest?.userId, jobId: manifest?.jobId, basePath: captionBase, words, workDir })

  // Captions SIEMPRE al FINAL → quedan encima de las imágenes IA (regla inamovible).
  if (words.length > 0) {
    await burnCaptions({ rawPath: captionBase, words, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, subtitlePosition: manifest?.subtitlePosition, captionReplacements: manifest?.captionReplacements, whatsappNumber: manifest?.whatsappNumber, contacts: manifest?.contacts, visualStyle: manifest?.visualStyle, script: manifest?.script, description: manifest?.description, lang: manifest?.lang, impactTitles: manifest?.impactTitles })
  } else {
  ...
  if (sfxEnabled) {
    await applySfxPass({ outputPath, words, workDir, totalDur: voiceDur, logPrefix: '[render][create]' })
  }
...
async function renderEdit({ workDir, openaiKey, manifest }) {
  ...
  if (words.length > 0) {
    try {
      ...
      if (finalWords.length > 0) {
        // Base limpia (post content-trim, pre-captions) para poder corregir SOLO
        // subtítulos después sin rehacer el video completo. No bloqueante.
        await persistPrecaptionsBase({ userId: manifest?.userId, jobId: manifest?.jobId, basePath: finalRawPath, words: finalWords, workDir })
        await burnCaptions({ rawPath: finalRawPath, words: finalWords, outputPath, workDir, subtitleStyle: manifest?.subtitleStyle, subtitlePosition: manifest?.subtitlePosition, captionReplacements: manifest?.captionReplacements, whatsappNumber: manifest?.whatsappNumber, contacts: manifest?.contacts, visualStyle: manifest?.visualStyle, script: manifest?.script, description: manifest?.description, lang: manifest?.lang, impactTitles: manifest?.impactTitles })
      } else {
    ...
    try {
      ...
      if (moments.length > 0) {
        ...
        if (ok.length > 0) {
          ...
          // WS13: títulos de impacto (si burnCaptions los generó) DEBAJO de los subtítulos,
          // ambos ENCIMA de las imágenes IA. Subtítulos SIEMPRE al frente (inamovible #25).
          try {
            const titAss = path.join(workDir, 'titles.ass')
            await readFile(titAss)
            fc3.push(`${lastLabel}ass='${ffmpegAssPath(titAss)}'[vtit]`)
            lastLabel = '[vtit]'
            mapLabel = '[vtit]'
          } catch { /* sin titles.ass → no hay títulos de impacto */ }
          try {
  ...
  if (sfxEnabled) {
    await applySfxPass({ outputPath, words, workDir, totalDur, logPrefix: '[render][edit]' })
  }
...
async function burnCaptions({ rawPath, words, outputPath, workDir, subtitleStyle, subtitlePosition, captionReplacements, whatsappNumber, contacts, visualStyle, script, description, lang, impactTitles }) {
  ...
  // ── WS13 · Títulos de impacto (kinetic text) ───────────────────────────────
  // Frases de IMPACTO de la narración, GRANDES y animadas arriba-centro, en su momento.
  // DISTINTO de los subtítulos (esos van abajo, todas las palabras). Regla de marca del
  // producto: SIEMPRE on (opt-out técnico manifest.impactTitles==='off'). Se quema DEBAJO
  // de los subtítulos (los subtítulos SIEMPRE al frente, inamovible #25). Fallback total:
  // si el LLM no devuelve nada, no se agrega capa (el video sale igual que antes).
  const titlesPath = path.join(workDir, 'titles.ass')
  let titlesEscaped = null
  if (impactTitles !== 'off') {
    try {
      const titles = await detectImpactTitles(fixedWords, process.env.ANTHROPIC_API_KEY, {
        script, description, lang,
      })
      const ass = buildTitlesASS(titles, { W, H })
      if (ass) {
        await writeFile(titlesPath, ass, 'utf8')
        titlesEscaped = ffmpegAssPath(titlesPath)
        console.log(`[render] impact titles: ${titles.map((t) => `"${t.text}"@${t.startSec.toFixed(1)}s`).join(', ')}`)
      }
    } catch (e) {
      console.warn('[render] impact titles skipped:', e?.message ?? e)
    }
  }
  ...
  // `hasTitles` decide el piso de seguridad: si esta corrida no lleva títulos de
  // impacto, los subtítulos pueden subir un poco más sin riesgo de pisarlos.
  await writeFile(
    captionsPath,
    buildASS(fixedWords, subtitleStyle, [], subtitlePosition, { hasTitles: Boolean(titlesEscaped) }),
    'utf8',
  )
  ...
  if (contactOverlays.length) {
    ...
    // TÍTULOS DE IMPACTO debajo, SUBTÍTULOS al FINAL encima de TODO (inamovible #25).
    if (titlesEscaped) {
      fc.push(`${lastLabel}ass='${titlesEscaped}'[vt]`)
      fc.push(`[vt]ass='${escaped}'[v]`)
    } else {
    ...
    // Títulos de impacto (si hay) DEBAJO de los subtítulos (encadenados, captions al frente).
    const vf = titlesEscaped ? `ass='${titlesEscaped}',ass='${escaped}'` : `ass='${escaped}'`
    cmd2 = [
      'ffmpeg -y',
      `-i ${q(rawPath)}`,
      `-vf "${vf}"`,
...
// AI-driven SFX (whoosh/ding/boom/pop/sparkle/swoosh/click) — extraído de renderCreate
// y renderEdit (eran bloques casi idénticos) para poder reusarlo también desde
// runCaptionsFix. Mezcla los efectos en el audio de `outputPath` IN PLACE. Reglas
// profesionales de sound design en llm-sfx.js. Fallback graceful: nunca lanza.
async function applySfxPass({ outputPath, words, workDir, totalDur, logPrefix }) {
  try {
    ...
    // `used` evita repetir el mismo archivo de SFX dentro de este render (variedad).
    const used = new Set()
    const sfxPlacements = sfxMoments
      .map((m) => ({ ...m, filePath: pickVariedSFXFile(m.category, catalog, used) }))
      .filter((m) => m.filePath !== null)

    if (sfxPlacements.length === 0) {
      console.log(`${logPrefix} pass 4 skipped: no SFX placements from LLM`)
      return
    }

    console.log(`${logPrefix} pass 4: mixing ${sfxPlacements.length} SFX into audio...`)
    ...
    sfxPlacements.forEach((sfx, i) => {
      ...
      fc4Parts.push(`[${i + 1}:a]adelay=${sfx.delay_ms}|${sfx.delay_ms},volume=${sfx.volume}[sfx${i}]`)
    ...
    console.log(`${logPrefix} pass 4 cmd (300):`, cmd4.slice(0, 300))
    await execAsync(cmd4, { maxBuffer: 300 * 1024 * 1024, timeout: 120_000 })
    await copyFile(withSfxPath, outputPath)
    console.log(`${logPrefix} pass 4 done — ${sfxPlacements.length} SFX applied`)
  } catch (e) {
    console.warn(`${logPrefix} pass 4 (SFX) FAILED, keeping clean output: ${e?.message ?? e}`)
  }
}

// Sube la base LIMPIA (post recursos/content-trim, ANTES de quemar captions) + el
// transcript, para que una corrección de SOLO subtítulos (runCaptionsFix) pueda
// re-quemar captions sin rehacer voz/ilustraciones/avatar/música/content-trim.
// No bloqueante: si falla (o falta userId/jobId), el render principal sigue igual —
// simplemente ese job no tendrá disponible la corrección liviana.
async function persistPrecaptionsBase({ userId, jobId, basePath, words, workDir }) {
  if (!userId || !jobId) return
  try {
    await uploadAsset(userId, jobId, basePath, 'precaptions.mp4', 'video/mp4')
    const wordsPath = path.join(workDir, 'words.json')
    await writeFile(wordsPath, JSON.stringify(words), 'utf8')
    await uploadAsset(userId, jobId, wordsPath, 'words.json', 'application/json')
  } catch (e) {
    console.warn('[render] persistPrecaptionsBase failed (non-blocking):', e?.message ?? e)
  }
}

/**
 * Corrección de SOLO SUBTÍTULOS sobre un video YA renderizado — NO rehace el video
 * completo (no vuelve a generar voz, ilustraciones IA, avatar, ni música). Reusa la
 * base "precaptions.mp4" + el transcript "words.json" que persistió el render
 * original (persistPrecaptionsBase), re-quema captions/títulos de impacto/contacto
 * con las correcciones nuevas, vuelve a mezclar SFX y a agregar intro/outro —
 * exactamente el mismo resultado que un render completo, en una fracción del tiempo
 * y sin ninguna llamada a ElevenLabs/KIE/Whisper.
 *
 * Lanza `no_precaptions_asset` si el job es de ANTES de esta función (no tiene la
 * base persistida) — el caller (server.js → SaaS) debe caer a un render completo
 * en ese caso; los renders desde acá en adelante SÍ la tienen.
 */
export async function runCaptionsFix({ workDir, userId, jobId, newReplacements, subtitlePosition }) {
  ...
  const basePath = path.join(workDir, 'precaptions.mp4')
  const wordsPath = path.join(workDir, 'words.json')
  try {
    await downloadOneAsset(userId, jobId, 'precaptions.mp4', basePath)
    await downloadOneAsset(userId, jobId, 'words.json', wordsPath)
  } catch {
    const err = new Error('no_precaptions_asset')
    err.code = 'no_precaptions_asset'
    throw err
  }
  const words = JSON.parse(await readFile(wordsPath, 'utf8'))

  // Merge + dedupe por `from` (case-insensitive) — la corrección nueva pisa a una
  // vieja para la misma palabra; se acumulan las de distintas palabras.
  const existing = Array.isArray(manifest?.captionReplacements) ? manifest.captionReplacements : []
  const merged = [...existing]
  for (const r of (Array.isArray(newReplacements) ? newReplacements : [])) {
    if (!r?.from || !r?.to) continue
    const i = merged.findIndex((m) => String(m.from).trim().toLowerCase() === String(r.from).trim().toLowerCase())
    if (i >= 0) merged[i] = r
    else merged.push(r)
  }
  ...
  // Mover los subtítulos de un video YA renderizado es exactamente el mismo
  // trabajo que corregirles una palabra: re-quemar sobre la base limpia. Si el
  // pedido trae una posición nueva, gana sobre la del manifest y se persiste.
  const effPosition = subtitlePosition || manifest?.subtitlePosition

  const outputPath = path.join(workDir, 'output.mp4')
  await burnCaptions({
    rawPath: basePath, words, outputPath, workDir,
    subtitleStyle: manifest?.subtitleStyle, subtitlePosition: effPosition, captionReplacements: merged,
    whatsappNumber: manifest?.whatsappNumber, contacts: manifest?.contacts,
    visualStyle: manifest?.visualStyle, script: manifest?.script,
    description: manifest?.description, lang: manifest?.lang, impactTitles: manifest?.impactTitles,
  })

  if (manifest?.sfx !== 'off' && words.length > 0) {
    const totalDur = await getMediaDuration(basePath).catch(() => 0)
    if (totalDur > 0) await applySfxPass({ outputPath, words, workDir, totalDur, logPrefix: '[render][captions-fix]' })
  }

  await addIntroOutro(outputPath, logoPath, workDir, { W, H })

  const publicUrl = await uploadOutput(userId, jobId, outputPath, 'output.mp4')
  // Re-persistir la base + transcript (por si el propio outputPath se sobreescribió
  // encima de precaptions.mp4 en algún paso — no debería, pero es defensivo/barato)
  // y el manifest con las correcciones acumuladas, para que un futuro render
  // completo (rerender "otro formato", etc.) también las herede.
  await writeFile(
    path.join(workDir, 'job.json'),
    JSON.stringify({ ...manifest, captionReplacements: merged, subtitlePosition: effPosition }, null, 2),
    'utf8',
  )
  await uploadAsset(userId, jobId, path.join(workDir, 'job.json'), 'job.json', 'application/json')

  return { outputPath, publicUrl, mergedReplacements: merged }
}
