 ...
 */
// 2026-07-18 (Javier: "no le pusieron música y seleccioné música"): diagnóstico
// real contra Kie confirmó `GENERATE_AUDIO_FAILED` con errorCode 500 ("Internal
// Error, Please try again later.") — un fallo TRANSITORIO del lado de Kie/Suno,
// no de nuestra key ni del prompt. Antes de este fix, un solo 500 tiraba toda la
// música del render sin avisar a nadie. Reintentamos el intento COMPLETO
// (create+poll) hasta MUSIC_RETRIES veces ante errores que huelen a transitorios.
const MUSIC_RETRIES = 2

export async function generateMusicFromPrompt(prompt, opts = {}) {
  let lastErr = null
  for (let attempt = 1; attempt <= MUSIC_RETRIES + 1; attempt++) {
    try {
      return await generateMusicOnce(prompt, opts)
    } catch (e) {
      lastErr = e
      const msg = e?.message ?? String(e)
      const isSensitive = msg.includes('SENSITIVE_WORD_ERROR')
      if (isSensitive || attempt > MUSIC_RETRIES) throw e   // no reintentar contenido rechazado ni agotar más intentos
      console.warn(`[kie-music] intento ${attempt} falló (${msg.slice(0, 150)}), reintentando...`)
...
async function generateMusicOnce(prompt, opts = {}) {
