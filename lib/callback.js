...
export async function postCallback({ jobId, userId, status, videoUrl, thumbnailUrl, coverUrl, coverTitle, coverFields, coverFrames, stats, error }) {
  if (!CALLBACK_URL) {
    console.warn('[callback] CALLBACK_URL not set, skipping')
    return
  }
  const body = JSON.stringify({ jobId, userId, status, videoUrl, thumbnailUrl, coverUrl, coverTitle, coverFields, coverFrames, stats, error })
  try {
