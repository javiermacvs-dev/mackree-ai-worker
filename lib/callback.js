...
export async function postCallback({ jobId, userId, status, videoUrl, thumbnailUrl, error }) {
  if (!CALLBACK_URL) {
    console.warn('[callback] CALLBACK_URL not set, skipping')
    return
  }
  const body = JSON.stringify({ jobId, userId, status, videoUrl, thumbnailUrl, error })
  try {
