...
export async function generateContactIllustration({ kind, value, style, workDir, apiKey, W, H, index = 0 }) {
  try {
    ...
    const yPos = Math.round(H * 0.12) // franja inferior

    const out = path.join(workDir, `contact_${channel}_${index}.jpg`)
    const drawtext = FONTFILE
      ? `,drawtext=fontfile='${fwd(FONTFILE)}':text='${safe}':fontsize=${fontsize}:fontcolor=${fontcolor}:borderw=6:bordercolor=${bordercolor}:x=(w-text_w)/2:y=h-${yPos}`
      : ''
