...
// ⚠️ SEGURIDAD (CN-001): usamos execFile (SIN shell) — el texto de contacto lo controla
// el usuario y antes iba a `exec` (=/bin/sh -c) → command injection / RCE. Con execFile
// los metacaracteres de shell ($, `, ", ;, |, …) quedan inertes: no hay shell que los
// interprete. safeText además aplica una whitelist estricta (defensa en profundidad para
// el parser de filtros de ffmpeg). NO volver a `exec`/`execSync` con datos de usuario aquí.
...
/**
 * Sanea el dato para drawtext. Whitelist ESTRICTA (CN-001): solo letras/dígitos/espacio y
 * los símbolos válidos en teléfonos/emails/dominios (@ . + - _ / ( ) espacio). Todo lo demás
 * —incluidos `:` `'` `\` `%` `"` `$` `` ` `` `;` `|` &c.— se elimina. Esto neutraliza tanto la
 * inyección al parser de filtros de ffmpeg como cualquier metacaracter de shell (aunque ya
 * usamos execFile sin shell). Tope de 80 chars.
 */
function safeText(s) {
  return String(s || '')
...
export async function generateContactIllustration({ kind, value, style, workDir, apiKey, W, H, index = 0 }) {
  try {
    ...
    // execFile (sin shell): cada token es un argumento aislado; `vf` va como UN solo
    // argumento y ffmpeg lo parsea internamente (no hay /bin/sh que interprete `safe`).
    await execFileAsync('ffmpeg', ['-y', '-i', raw, '-vf', vf, out], { timeout: 60000 })
