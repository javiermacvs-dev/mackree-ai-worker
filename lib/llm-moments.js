...
export async function detectScriptScenes(words, anthropicKey, opts = {}) {
  ...
  // 'auto' no está en STYLES: Chixy elige la mejor estética según el guión.
  // Usamos 'doodle' (sobrio, el de Santiago) como selección por defecto para 'auto'.
  // La lógica de elección IA se puede expandir aquí en el futuro.
  const rawStyle = opts.visualStyle === 'auto' ? DEFAULT_STYLE : opts.visualStyle
  const visualStyle = normalizeStyle(rawStyle)
  const styleLabel = (STYLES[visualStyle] || STYLES[DEFAULT_STYLE]).label
  const transcript = words.map((w) => `[${(w.start ?? 0).toFixed(2)}s] ${w.word}`).join(' ')
