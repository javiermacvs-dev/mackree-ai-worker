...
28. **WS7 — Control de DURACIÓN (respetar el tiempo elegido)** — **v62** ✅ (Javier 2026-06-07). El video en modo voz (`renderCreate`: "editar video con voz" + "generar con IA") debe respetar la duración elegida con un **PISO DURO = target (nunca menos)** y techo blando = target+4. Helper `adjustVoiceToTarget(voicePath, voiceDur, targetSec, workDir)` (`lib/render.js`, tras `getMediaDuration`) ajusta la VOZ (master de duración) ANTES del pipeline → afecta el video entero sin tocar las ~10 referencias a `voiceDur`: **voz > target+4 → `atempo` tope 1.10x** (calidad; preserva el tono); **voz < target → `apad`** (rellena con silencio/música hasta el piso, NO toca la voz); **voz ∈ [target,target+4] → sin tocar**. Target = `parseInt(manifest.duration)`; si inválido (0) → no toca nada (back-compat). **`renderEdit` (modo "editar mi video") NO se toca** (duración = footage). **Decisiones de Javier (INAMOVIBLES):** piso duro nunca menos que el objetivo; aceleración máx **1.10x**; calidad primero (preferir apad que atempo). La calibración del guión (SaaS `script/route.ts`) apunta al target con ritmo conservador (~2.2 p/s) para que domine el apad. **Validado interno:** piso 5/5 + E2E voz real 3/3 en rango (15→19, 30→34, 60→63.7s, atempo 1-4% imperceptible). **NO bajar el tope 1.10x ni quitar el piso sin pedido de Javier.** **Pendiente: validación de Javier en render real (modos 2 y 3).**
...
| Build version | leer `BUILD_VERSION` en `server.js` (actual: `v62-duration-control`) |
