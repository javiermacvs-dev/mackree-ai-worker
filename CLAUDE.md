...
> **📌 PRÓXIMA SESIÓN (Javier 2026-05-24):** va a probar el flujo **EDIT mode** ("Editar mi video" — usa el audio/voz ORIGINAL del clip, sin narración TTS). ⚠️ Recordatorios al revisar edit mode: (a) el ajuste de **voz `1.45` de v45 es SOLO create** (narración); en edit la voz viene del footage con `loudnorm I=-16` — si Javier pide subir la voz en edit, el dial es loudnorm I (-16→-14), no el `volume`; (b) **transiciones xfade (v37) y Ken Burns (v35) todavía SOLO en create** — en edit los cortes son duros (pendiente portar, requiere acrossfade de audio + offsets de captions); (c) intro/outro (v43), animaciones simples 9:16 (v46), música comercial `0.09` (v45), captions, SFX y corrección de nombres de marca SÍ aplican a edit.
...
| Build version | leer `BUILD_VERSION` en `server.js` (actual: `v46-simple-images-vertical`) |
