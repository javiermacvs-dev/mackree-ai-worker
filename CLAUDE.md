...
20. **CTA de WhatsApp animado** — **v40** ✅ Cuando el video menciona WhatsApp y la empresa tiene `whatsapp_number`, `burnCaptions` inserta un badge verde animado (caja #25D366 = `&H0066D325`, texto blanco, fade + slide-up) con el número, en el momento de la mención (fallback: cerca del cierre si Whisper no captó "WhatsApp"). Estilo `WA` + evento `\fad\move` en `buildASS`. `manifest.whatsappNumber` viene de `brand.whatsapp_number` (Mac Gyver = `+1 (407) 969 7969`). Sin número configurado → no se muestra.
...
- **Estilo visual de las ilustraciones IA** (v30, ampliado v49 2026-05-31) — **10 estilos**: `doodle` (default, el de Santiago) / `whiteboard` / `flat` / `isometric` / `claymation` / `watercolor` / `comic` (Cómic/Pop-art) / `lineart` (Línea minimalista) / `render3d` (3D Pixar) / `neon` (Neón/Cyberpunk). Es preferencia estética → dropdown en el SaaS, llega en `manifest.visualStyle`. Para agregar uno nuevo tocar 4 lugares: `lib/styles.js` (worker, prompt_base SIN orientación) + skill `~/.claude/skills/mackree-styles/style-catalog.json` (CON orientación, alimenta los thumbnails 16:9) + `chixy/src/lib/visual-styles.ts` (tipo + array ES) + generar/subir thumbnails (`EDIT_VIDEO/style-samples/gen_style_cards.mjs` con su `NICHE_SUBJECTS` + `upload_thumbnails.mjs` al bucket `style-thumbnails`). El worker lo aplica en `lib/styles.js` + `lib/llm-moments.js` (LLM devuelve solo el sujeto) + `lib/kie-image.js` (prepend del `prompt_base`). Reemplaza el viejo "cinematic photorealistic automotive" hardcodeado. Default `doodle` si el manifest no lo trae. Aplica en AMBOS flujos (edit + create con voz).
...
| Build version | leer `BUILD_VERSION` en `server.js` (actual: `v48-full-ai-canvas`) |
