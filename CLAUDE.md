...
21. **Imágenes IA SIEMPRE derechas (16:9 + card centrada)** — **v41→v42** ✅ Las ilustraciones doodle se generan en **16:9** (nano-banana las dibuja DERECHAS; en 9:16 acostaba el contenido 90° → texto de lado, bug crítico Javier 2026-05-24 "sale volteada, se ve horrible") y se montan como **card centrada con fondo difuminado** (`split` + `boxblur` + overlay centrado), sin rotar ni recortar. ⚠️ El prompt-only fix (v41: quitar "horizontal" de `styles.js` + inyectar orientación en `kie-image.js`) **NO bastó** — nano-banana acuesta las infografías igual. La solución determinista es **generar en 16:9 + card** (v42). Cableado en `lib/render.js` pass 3 (create + edit) + `lib/kie-image.js` (`aspectRatio:'16:9'` desde las llamadas) + `lib/styles.js` (prompt_base sin orientación). **NO volver a 9:16 fullscreen para las imágenes IA.**
